...
const btnCartAlert = document.querySelectorAll('.btn-add-cart')
...
async function manejarClic() {
    ...
    const { value: color } = await Swal.fire({
        ...
        inputValidator: (value) => {
            if (!value) {
                return "Tienes que seleccionar uno!";
            }
        }
    });
...
const productoJson = './data.json';

function obtenerInfo() {
    return new Promise(async (resolve, reject) => {
        try {
            const response = await fetch(productoJson);
            if (!response.ok) {
                throw new Error('Error en cargar el producto')
            }
